...
ARGBImage read_argb(const std::string &filename_)
{
    ...
    uint8_t *rgba_image(
        stbi_load(filename.c_str(), &width_signed, &height_signed, &bpp, CHANNEL_NUM_ARGB));
...
void write_argb(const ARGBImage &argb_image, const std::string &filename)
{
    ...
    stbi_write_png(filename.c_str(),
                   ...
                   CHANNEL_NUM_ARGB,
                   rgba_image,
                   static_cast<int>(width) * CHANNEL_NUM_ARGB);
    stbi_image_free(rgba_image);
}
...
uint8_t luminance(uint32_t argb)
{
    ...
    return static_cast<uint8_t>(std::round(
        LUMA_COEFFICIENT_RED * red + LUMA_COEFFICIENT_GREEN * green + LUMA_COEFFICIENT_BLUE * blue));
}
...
ARGBImage binary_diff_to_argb(const BinaryImage &binary_image1, const BinaryImage &binary_image2)
{
    ...
    for (size_t i(0); i < height; i++) {
        for (size_t j(0); j < width; j++) {
            if (binary_image1[i][j] != binary_image2[i][j])
                argb_image[i][j] = 0xFFFF0000;
            else
...
void add_line(ARGBImage &argb_image,
              int row_start,
              int column_start,
              double orientation,
              unsigned int length,
              uint32_t color)
{
    ...
    int row_end(
        static_cast<int>(std::round(row_start - static_cast<int>(length) * std::sin(orientation))));
    int column_end(static_cast<int>(
        std::round(column_start + static_cast<int>(length) * std::cos(orientation))));
    ...
    if (std::abs(d_row) <= std::abs(d_column)) {
        ...
        while (column != column_end) {
            ...
            if (0 <= r && r < static_cast<int>(height) && 0 <= column
    ...
    if (0 <= row_end && row_end < static_cast<int>(height) && 0 <= column_end
        && column_end < static_cast<int>(width))
        argb_image[static_cast<size_t>(row_end)][static_cast<size_t>(column_end)] = color;
}

void add_circle(
    ARGBImage &argb_image, int row_center, int column_center, unsigned int radius, uint32_t color)
{
    size_t height(argb_image.size());
    size_t width(argb_image[0].size());
    ...
    for (unsigned int i(0); i < 360; ++i) {
        int row = row_center
                  + static_cast<int>(
                      std::round(static_cast<int>(radius) * std::sin(degrees_to_radians(i))));
        int column = column_center
                     + static_cast<int>(
                         std::round(static_cast<int>(radius) * std::cos(degrees_to_radians(i))));

        if (0 <= row && row < static_cast<int>(height) && 0 <= column
            && column < static_cast<int>(width))
...
void draw_minutia(ARGBImage &argb_image, const std::vector<Minutia> &minutiae)
{
    for (const Minutia &minutia : minutiae) {
        ...
        add_line(argb_image,
                 minutia.row,
                 minutia.column,
                 degrees_to_radians(minutia.angle_in_degrees),
                 8,
                 0xFFFF0000);
    }
}
