...
//   Voir -> compute_angle()
//
// -- Optimisation de la comparaison des distances (Euclidiennes)
//
...
// - Reddit.com (en grande partie pour la nomenclature C++)
// - Forum Ed discussion (Pour certaines optimisations)
...
BinaryImage connected_pixels(const BinaryImage &binary_image, size_t row, size_t column, unsigned int distance) {
    ...
    //On calcule les limite de recherches
    int indentation_start_row = int_row - int_distance;
    int indentation_end_row = int_row + int_distance;
    int indentation_start_column = int_column - int_distance;
    int inndentation_end_column = int_column + int_distance;

    int max_row = static_cast<int>(binary_image.size()) - 1;
    int max_column = static_cast<int>(binary_image[0].size()) - 1;

    //On implemente les exeption au cas ou on est au bord de l image
    if (indentation_start_row < 0){
        indentation_start_row = 0;
    }
    if (indentation_start_column < 0){
        indentation_start_column = 0;
    }
    if (indentation_end_row > max_row){
        indentation_end_row = max_row;
    }
    if (inndentation_end_column > max_column){
        inndentation_end_column = max_column;
    }
    ...
    while (loop) {
        loop=false;
        for (int i(indentation_start_row);i<=indentation_end_row; i++) {
            for (int j(indentation_start_column); j <= inndentation_end_column; ++j) {
                ...
                if(!output[i][j]
                    ...
                    if(
                        ...
                        or (i<max_row and output[i+1][j])
                        or (j>0 and output[i][j-1])
                        or (j<max_column and output[i][j+1])
                        or ((j<max_column and i<max_row) and output[i+1][j+1])
                        or ((j>0 and i>0) and output[i-1][j-1])
                        or ((i<max_row and j>0) and output[i+1][j-1])
                        or ((j<max_column and i>0) and output[i-1][j+1])){
                        output[i][j]=true;
                        loop=true;
                    }
                }
...
// Calcul les coordonnée relative des pixels adjacents à la minutie.
vector<Coord> compute_relative_coordinate(const BinaryImage &binary_image, size_t row, size_t column){
    ...
    int indentation_start_row = int_row - max_distance;
    int indentation_end_row = int_row + max_distance + 1;
    int indentation_start_column = int_column - max_distance;
    int indentation_end_column = int_column + max_distance + 1;

    const size_t row_size = binary_image.size();
    const size_t column_size = binary_image[0].size();
    ...
    if (indentation_end_row > row_size) {
        indentation_end_row = row_size;
    }
    if (indentation_end_column > column_size) {
        indentation_end_column = column_size;
    }

    size_t start_i = to_size_t(indentation_start_row);
    size_t start_j = to_size_t(indentation_start_column);
    size_t end_i = to_size_t(indentation_end_row);
    size_t end_j = to_size_t(indentation_end_column);
...
Minutia apply_rotation(const Minutia &minutia, int center_row, int center_column, int rotation_in_degrees) {
    ...
    // Coordonnées 'x' relative à la minutie.
    int x = minutia.column - center_column;
    // Coordonnées 'y' relative à la minutie.
    int y = center_row - minutia.row;
    // Nouvelle coordonnée 'x' après avoir appliqué la rotation.
    double new_x = x*cos_rotation-y*sin_rotation;
    //Nouvelle coordonnée 'y' après avoir appliqué la rotation.
    double new_y = x*sin_rotation+y*cos_rotation;
    //Calcul de la nouvelle coordonnée absolue de la ligne.
    int new_row = static_cast<int>(round(center_row-new_y));
    //Calcul de la nouvelle coordonnée absolue de la colonne.
...
Minutia apply_translation(const Minutia &minutia, int row_translation, int column_translation) {
    //Calcul de la nouvelle ligne après translation.
    int new_row = minutia.row-row_translation;
    // Calcul de la nouvelle colonne après translation.
...
bool match(const std::vector<Minutia> &minutiae_1, const std::vector<Minutia> &minutiae_2) {
    ...
    for (size_t a = 0; a < minutiae1_size; ++a) {
        ...
        for (size_t b = 0; b < minutiae2_size; ++b) {
            ...
            // Longueur de la translation verticale
            int row_trans = minutia2_row-minutia1_row;

            // Longueur de la translation horizontale
            int column_trans = minutia2_col-minutia1_col;

            // Différence d'orientation entre minutie 1 et minutie 2
