...
bool operator==(const Minutia &minutia_1, const Minutia &minutia_2) {
    return minutia_1.row == minutia_2.row
        && minutia_1.column == minutia_2.column
        && minutia_1.angle_in_degrees == minutia_2.angle_in_degrees;
}

bool operator==(const BinaryImage& image_1, const BinaryImage& image_2){
    if (image_1.size() != image_2.size()) return false;
    for (size_t row(0); row < image_1.size(); ++row){
        if (image_1[row].size() != image_2[row].size()) return false;
        for (size_t col(0); col< image_1[row].size(); ++col){
            if (image_1[row][col] != image_2[row][col]) return false;
        }
    }
    return true;
}
...
std::string minutia_to_string(const Minutia &minutia) {
  std::ostringstream ss;
  ss << "{row: " << minutia.row << ", column: " << minutia.column << ", angle_in_degrees: " << minutia.angle_in_degrees << "}" << std::endl;
  return ss.str();
}
...
void print_box(const std::string &text, const std::string &top_left, const std::string &top_right, const std::string &bottom_left, const std::string &bottom_right, const std::string &horizontal, const std::string &vertical, size_t min_width = 50) {
...
void print_part_header(unsigned int part) {
  if (part < 1 || part > 3) throw std::invalid_argument("Invalid part number");
...
void print_assertion_result(bool passes, const std::string &expected, const std::string &actual) {
  if (passes) std::cout << "--- OK ---" << std::endl;
...
void print_file_comparison_result(bool passes, const std::string &expected_filename, const std::string &actual_filename) {
